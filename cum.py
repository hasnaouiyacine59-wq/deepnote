...
parser.add_argument('--socks-port',   type=int, default=None, help='Tor SOCKS5 port (overrides SOCKS_PORT env)')
parser.add_argument('--control-port', type=int, default=None, help='Tor control port for IP rotation')
parser.add_argument('-T', action='store_true', help='(ignored, compatibility flag)')
...
# Tor proxy settings (read from env or use defaults, CLI args take priority)
TOR_HOST    = os.getenv('TOR_HOST',    '127.0.0.1')
SOCKS_PORT  = args.socks_port   if args.socks_port   else int(os.getenv('SOCKS_PORT',   '9050'))
CONTROL_PORT= args.control_port if args.control_port else int(os.getenv('CONTROL_PORT', '9051'))
API_PORT    = int(os.getenv('API_PORT', '5000'))  # kept for optional external API compat

PROXY     = f'socks5://{TOR_HOST}:{SOCKS_PORT}'
IP_API    = f'http://{TOR_HOST}:{API_PORT}/ip'        # get current exit IP (optional external API)
RESET_API = f'http://{TOR_HOST}:{API_PORT}/reset-ip'  # trigger IP rotation (optional external API)
...
def _tor_controller():
    """Return an authenticated stem Controller for the Tor control port."""
    c = Controller.from_port(address=TOR_HOST, port=CONTROL_PORT)
    c.authenticate()  # works with HashedControlPassword "" (no password)
    return c

def reset_ip():
    """Send NEWNYM signal to Tor to rotate to a new exit circuit."""
    try:
        with _tor_controller() as c:
            c.signal(Signal.NEWNYM)
    except Exception as e:
        print(f"[!] reset_ip error: {e}")
        # Fallback: try the external API if stem fails
        try:
...
def _fetch_ip(retries=12, delay=5):
    """Fetch the current Tor exit IP.
    Tries stem first (via check.torproject.org through the SOCKS proxy),
    falls back to the external API if available.
    """
    for attempt in range(retries):
        try:
            proxies = {'http': PROXY, 'https': PROXY}
            r = requests.get('https://check.torproject.org/api/ip', proxies=proxies, timeout=10).json()
            return r.get('IP', '0.0.0.0')
        except Exception:
            pass
        # Fallback: try the optional external API
        try:
            return requests.get(IP_API, timeout=10).json().get('ip', '0.0.0.0')
        except Exception as e:
            print(f"[*] IP fetch not ready ({e}), retrying in {delay}s... ({attempt+1}/{retries})")
            time.sleep(delay)
    raise RuntimeError(f"Could not fetch Tor exit IP after {retries} attempts")
...
# Poll the Tor control port via stem until bootstrap reaches 100%
print("[*] Waiting for Tor to bootstrap...")
for _ in range(30):
    try:
        with _tor_controller() as c:
            phase = c.get_info('status/bootstrap-phase', '')
